...
---
# Referenzbeispiele für Fusion-Versionshinweise
...
Basierend auf `fusion-2026-6-22.md`.
...
Basierend auf `fusion-2026-3-9.md`.
...
Beginn eines ganz neuen Jahres (nur Beispiel - Führen Sie dies aus, wenn die erste Version von {YYYY+1} veröffentlicht wird):
...
Hinzufügen der Woche vom 20. Juli 2026 als neuester Eintrag:
...
1. **Fehlerhafte Jahresüberschrift in TOC.md** — Die Einträge vom Januar bis Februar 2026 befinden sich derzeit unter der `Fusion releases - 2025` Überschrift anstelle von `Fusion releases - 2026`. Dies ist ein bereits vorhandener Fehler, nicht die beabsichtigte Struktur.
2. **Fehlendes Komma vor Jahr** - Seiten wie `fusion-2026-7-13.md` verwenden „13. Juli 2026“ anstelle von „13. Juli 2026“ im Titel/in der Beschreibung/H1. Setzen Sie auf neuen Seiten immer ein Komma.
3. **`hidefromtoc`vs. `exl-id`/`TQID`** - Seiten nach oben bis `fusion-2026-4-27.md` tragen eine `exl-id` (und manchmal `TQID`); Seiten ab `fusion-2026-5-4.md` verwenden stattdessen `hidefromtoc: true`. Neue Seiten sollten dem neueren Muster folgen (`hidefromtoc: true`, keine `exl-id`/`TQID`), da diese Kennungen später von der Publishing-Pipeline zugewiesen werden.
4. **`{hide-from-toc}`Präfix in TOC.md** - Wird verwendet, um die Hervorhebung älterer Einträge in der gerenderten Navigation zu verringern, sobald sie aus der aktuellen Ansicht entfernt wurden. Fügen Sie dies nicht einem brandneuen Eintrag hinzu.
